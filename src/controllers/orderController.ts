...
type CheckOutSessionRequest = {
    ...
    restaurantId: string
}; //Fin de CheckOutSesionReques type

const createLineItems = (checkOutSessionRequest: CheckOutSessionRequest,
                         menuItems: MenuItemType[])=>{
    //1. Foreach carItem, obtenemos el objeto menuItem
    //   para el restaurante, para obtener el precio
...
export const createCheckOutSession = async (req: Request, res: Response): Promise<any>=>{
    ...
    // Creamos la nueva orden en Mongo
  ...
  createdAt: new Date(),
    ...
    // Creamos una sesión de Stripe — usamos el _id real de la orden
    ...
    if (!session.url) {
        return res.status(500)
            .json({ message: "Error al crear una sesión de Stripe" });
    }

    // Asignamos el totalAmount calculado por Stripe y guardamos
    ...
    console.log(error);
    console.log("Error al crear la sesión de Stripe");
    res.status(500)
        .json({ message: error.raw.message });
}
};//Fin de createCheckOutSession
...
export const getRestaurantOrders = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    console.log(error);
    res.status(500).json({ message: 'Error al obtener las ordenes para un restaurante' });
  }
};
export const updateOrderStatus = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const restaurant = await Restaurante.findById(order.restaurante._id);
    if (restaurant?._id.toString() !== order.restaurante?._id.toString()) {
